# 4myPown AI Agent - Autonomous Pentesting Framework
...
4myPown AI Agent is a fully autonomous penetration testing framework driven entirely by AI planning. Unlike traditional pentesting tools with hardcoded scripts, this system uses Large Language Models to dynamically plan, execute, and adapt reconnaissance and security testing based on real-time discoveries.
...
```bash
# CLI mode
python -m pown_cli.ai_orchestrator --target 127.0.0.1 --mode recon

# Interactive terminal UI
python -m pown_cli.ui.terminal_ui

# API server
python -m pown_cli.api_server
...
python -m pown_cli.ai_orchestrator \
...
from pown_cli.ai_providers import register_provider
...
### Adding New Agents
```python
from pown_cli.agents.base import BaseAgent
...
```python
from pown_cli.bus.events import get_publisher
...
python -c "from pown_cli.ai_providers import resolve_default_provider; print(resolve_default_provider().generate('test'))"
```
...
*4myPown AI Agent - Bringing AI-driven autonomy to penetration testing while maintaining safety and ethics.*