...
""".format(target=self.target, objective=self.objective))
    ...
    def _update_layout(self, layout: Layout, session_id: str):
        ...
        layout["header"].update(Panel(
            f"[bold cyan]4myPown AI Agent[/] - Session: {session_id}",
            style="cyan"
        ))
...
[dim]Press Ctrl+C to stop[/]"""
        ...
        layout["footer"].update(Panel(
            "[dim]4myPown AI Pentesting Agent - Streaming live reconnaissance results[/]",
            style="dim"
        ))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="4myPown Terminal UI")
